<?php 
$pageTitle = "Trang xoá kích thước";
...
require_once('../../controller/sizeController.php'); // File xử lý size

// Kiểm tra sự tồn tại của biến GET id
if (!isset($_GET['id'])) {
    header("Location: index.php?msg=ID size không hợp lệ.&type=failure");
    exit;
}

$size_id = trim($_GET['id']);
$size = getSizeById($conn, $size_id);
if (!$size) {
    header("Location: index.php?msg=Size không tồn tại.&type=failure");
    exit;
}

// Khi form được submit, gọi hàm xử lý xóa và chuyển hướng
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    processDeleteSize($conn, $size_id);
}
?>
...
<main class="container mx-auto p-6">
    ...
    <div class="flex justify-between items-center mb-4">
        <div>
            <h1 class="text-3xl font-bold">Xóa Kích Thước</h1>
            <p class="text-sm text-gray-500">ID size: <?= htmlspecialchars($size['size_id']) ?></p>
        </div>
    ...
    <div class="bg-white shadow rounded p-6">
        <p class="mb-4 text-lg">
            ...
            <strong><?= htmlspecialchars($size['size_name']) ?></strong>?
        </p>
