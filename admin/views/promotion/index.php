<?php 
$pageTitle = "Trang quản lý khuyến mãi";
...
// Gọi hàm lấy dữ liệu danh sách khuyến mãi có phân trang
$data = getPromotionsWithPagination($conn, $page, $limit, $search);
...
<main class="container mx-auto p-6">
    <!-- Header: Tiêu đề và nút thêm khuyến mãi -->
    <div class="flex justify-between items-center mb-4">
        <h1 class="text-3xl sm:text-4xl font-bold text-gray-800">Danh Sách Khuyến Mãi</h1>
        <a href="add.php"
            ...
            <span class="hidden sm:inline">Thêm Khuyến Mãi</span>
        </a>
    </div>
    ...
    <div class="flex justify-end mb-4">
        <form method="GET" class="flex">
            <input type="text" name="search" value="<?= htmlspecialchars($search) ?>"
                class="p-2 border border-gray-300 rounded-l-lg focus:outline-none" placeholder="Tìm khuyến mãi...">
        ...
        <table class="w-full border-collapse text-sm sm:text-base">
            <thead class="sticky top-0 z-10">
                <tr class="bg-indigo-500 text-white">
                    <!-- STT hiển thị trên màn hình md trở lên -->
                    <th class="p-2 sm:p-3 hidden md:table-cell text-left">STT</th>
                    <!-- Tên khuyến mãi hiển thị luôn -->
                    <th class="p-2 sm:p-3 text-left">Tên Khuyến Mãi</th>
                    <!-- Mã khuyến mãi hiển thị luôn -->
                    <th class="p-2 sm:p-3 text-left">Mã Khuyến Mãi</th>
                    <!-- Giảm (%) hiển thị luôn -->
                    <th class="p-2 sm:p-3 text-right">Giảm (%)</th>
                    <!-- Các cột chỉ hiển thị trên màn hình lớn -->
                    <th class="p-2 sm:p-3 hidden lg:table-cell text-center">Bắt Đầu</th>
                    <th class="p-2 sm:p-3 hidden lg:table-cell text-center">Kết Thúc</th>
                    <!-- Cột Hành động luôn hiển thị (để cuối cùng) -->
            ...
            <tbody class="text-gray-700">
                ...
                <tr class="border-b <?= $rowClass ?> hover:bg-gray-200 transition">
                    <!-- STT: chỉ hiển thị trên md trở lên -->
                    <td class="p-2 sm:p-3 hidden md:table-cell align-middle"><?= $stt++ ?></td>
                    <!-- Tên khuyến mãi -->
                    <td class="p-2 sm:p-3 font-medium align-middle">
                        <?= htmlspecialchars($promotion['promotion_name']) ?></td>
                    <!-- Mã khuyến mãi -->
                    <td class="p-2 sm:p-3 align-middle"><?= htmlspecialchars($promotion['promotion_code']) ?></td>
                    <!-- Giảm (%) -->
                    <td class="p-2 sm:p-3 align-middle text-right"><?= number_format($promotion['discount_value'], 2) ?>
                    </td>
                    <!-- Bắt đầu: chỉ hiển thị trên lg trở lên -->
                    <td class="p-2 sm:p-3 hidden lg:table-cell align-middle text-center">
                        <?= htmlspecialchars($promotion['start_date']) ?></td>
                    <!-- Kết thúc: chỉ hiển thị trên lg trở lên -->
                    <td class="p-2 sm:p-3 hidden lg:table-cell align-middle text-center">
                        <?= htmlspecialchars($promotion['end_date']) ?></td>
                    <!-- Hành động luôn hiển thị, đặt cuối cùng -->
                    <td class="p-2 sm:p-3 text-center align-middle">
                        <div class="flex flex-col sm:flex-row gap-1 justify-center items-center">
                            <!-- Xem chi tiết -->
                            <a href="detail.php?id=<?= urlencode($promotion['promotion_id']) ?>"
                            ...
                            <!-- Chỉnh sửa -->
                            <a href="edit.php?id=<?= urlencode($promotion['promotion_id']) ?>"
                            ...
                            <!-- Xóa -->
                            <a href="delete.php?id=<?= urlencode($promotion['promotion_id']) ?>"
                                class="bg-red-200 hover:bg-red-300 w-10 h-10 flex items-center justify-center rounded-lg shadow-md transition"
                                onclick="return confirm('Bạn có chắc chắn muốn xóa khuyến mãi này?');" title="Xóa">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" fill="none"
                                    stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                    stroke-linejoin="round">
                                    <path d="M3 6h18"></path>
                ...
                <?php else : ?>
                <tr>
                    <td colspan="7" class="p-3 text-center text-gray-500">Không tìm thấy khuyến mãi nào.</td>
                </tr>
                <?php endif; ?>
            </tbody>
        </table>
