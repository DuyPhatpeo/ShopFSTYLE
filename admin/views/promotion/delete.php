<?php 
$pageTitle = "Trang xoá khuyến mãi";
...
require_once('../../controller/promotionController.php'); // File xử lý khuyến mãi

// Kiểm tra sự tồn tại của biến GET id
if (!isset($_GET['id'])) {
    header("Location: index.php?msg=ID khuyến mãi không hợp lệ.&type=failure");
    exit;
}

$promotion_id = trim($_GET['id']);
$promotion = getPromotionById($conn, $promotion_id);
if (!$promotion) {
    header("Location: index.php?msg=Khuyến mãi không tồn tại.&type=failure");
    exit;
}

// Khi form được submit, gọi hàm xử lý xóa và chuyển hướng
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    processDeletePromotion($conn, $promotion_id);
}
?>
...
<main class="container mx-auto p-6">
    ...
    <div class="flex justify-between items-center mb-4">
        <div>
            <h1 class="text-3xl font-bold">Xóa Khuyến Mãi</h1>
            <p class="text-sm text-gray-500">ID khuyến mãi: <?= htmlspecialchars($promotion['promotion_id']) ?></p>
        </div>
